...
│   ├── Todo/
│   │   ├── TodoItem.cs                # 待办数据模型（含 Progress/StartDate/DueDate/Children 子任务树）
...
│   │   ├── ChatWindow.xaml/.cs        # ⚠️ deprecated 旧版独立聊天窗口（仍被 MainWindow 引用）
...
│   │   ├── TodoWindow.xaml/.cs        # ⚠️ deprecated 旧版独立待办窗口（仍被 MainWindow 引用）
│   │   ├── CompactTodoWindow.xaml/.cs # ⚠️ deprecated 旧版迷你待办
│   │   └── BoolToStrikethroughConverter.cs
│   ├── HistoryView.xaml/.cs           # 历史视图（嵌入工作台 screenshots 页，4 列网格 + 时间筛选）
│   ├── SettingsView.xaml/.cs          # 设置视图（嵌入工作台 settings 页 + SettingsWindow）
│   ├── SettingsWindow.xaml/.cs        # 独立设置窗口（从迷你窗口右上角 ☰ 或托盘菜单打开）
│   ├── Options/OptionWindow.xaml/.cs  # ⚠️ deprecated 旧版设置窗口
│   ├── HistoryWindow.xaml/.cs         # ⚠️ deprecated 旧版历史窗口
...
- 底部按钮跳转：💬聊天 / ✅待办 / 📸截图 / ⚙设置
- 右上角 `☰` 三横按钮直接打开独立设置窗口 `SettingsWindow`
...
Layer 1: 全屏工作台 WorkbenchWindow / 独立设置窗口 SettingsWindow (普通窗口)
...
6. `SplashWindow` 当前未被 `App.OnStartup` 调用，保留作为后续启动加载流程使用（三态由 `SplashStatus` 驱动）
...
- **LLM 集成** — `TodoTools` 允许 LLM 增删改查（含子任务层级）

### 设置（双入口）

设置采用平铺式 Section 布局，由同一份 `SettingsView` UserControl 承载，两个入口：
- **工作台 `settings` 页** — 嵌入 WorkbenchWindow 主内容区
- **独立设置窗口 `SettingsWindow`** — 从迷你窗口右上角 `☰` 三横按钮或托盘菜单打开
